...
# Slice mode: bidirectional (TestRefiner full, default) / backward (ablation: w/o forward slicing, == TestWeaver) / forward (ablation)
# Overridable via env var SLICE_MODE or CLI --slice-mode (see __main__).
SLICE_MODE = os.getenv("SLICE_MODE", "bidirectional").lower()


def _slice(python_code, lineno, result_execute):
    """Dispatch slicing to the configured mode. Returns (filtered_code, _, filter_num_lines, _)."""
    mode = SLICE_MODE
    if mode == "forward":
...
def parse_args():
    ...
    ap.add_argument('--slice-mode', choices=['backward', 'forward', 'bidirectional'],
                    ...
                    help='slicing direction for context construction (default: bidirectional = full TestRefiner; backward = ablation "w/o forward slicing"; overrides SLICE_MODE env var)')
...
def testgeneration_feedback(client, prompt, epoch, install_missing=True):
    ...
    for i in range(epoch):
        ...
        # Diagnostic: log when LLM returns no usable content (None / empty)
        _msg = response.choices[0].message
        if _msg.content is None:
            _fr = response.choices[0].finish_reason
            print(f"[WARN] LLM returned content=None (finish_reason={_fr}) — skipping this round")
        generated_test = extract_python_code_block(_msg.content)
        if generated_test!="":
            # Check for missing imports and install them
            if missing := missing_imports(find_imports(generated_test)):
                print(f"Missing modules in generated test: {' '.join(missing)}")
...
# ======================== Phase 1: Basic test generation ========================
    ...
    miss_feedback = []
    missing_line_phase3 = missing_line_phase2  # Initialize in case Phase 3 loop doesn't execute run_evolution123
